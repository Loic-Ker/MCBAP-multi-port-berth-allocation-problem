...
function GRASP_reactive(seed,N,Nout,qli, type1, type2, type3, paramfixed, time_local, max_time_heur, max_time, expname, location)
    ...
    if feasible && checkSolutionFeasability(inst, sol)
        for iter in keys(d_alliter_after)
            #CSV.write(location*"results_jobs/benchmarks_HEUR/parametricGRASP/$expname"*"/iterations_before_local/sol_$seed"*"_$N"*"_$Nout"*"_$qli"*"/iter_$nb_iter"*"_beforelocal"*".csv", d_alliter_before[iter])
            #CSV.write(location*"results_jobs/benchmarks_HEUR/parametricGRASP/$expname"*"/iterations/sol_$seed"*"_$N"*"_$Nout"*"_$qli"*"/iter_$nb_iter"*".csv", d_alliter_after[iter])
        end
        print('\n')
