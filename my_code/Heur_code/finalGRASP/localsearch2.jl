...
function SelectNewVisitPerShipLocalSearch(inst::Instance, sol::Sol, n, c, p, paramfixed, reconstruct_no_improve)
    ...
    if paramfixed.lookforconstrained #&& reconstruct_no_improve<7
...
function SelectNewVisitLocalSearch(inst::Instance, sol::Sol, list_visits::Vector{Tuple}, paramchosen, paramfixed, reconstruct_no_improve) #, placed::Vector{Tuple{Int64,Int64}})
    ...
    for (n,c) in list_visits
        ...
        if sch[c].planned == false
            visit_constrained, feasible=SelectNewVisitPerShipLocalSearch(inst, sol, n, c, Pi[n][c], paramfixed, reconstruct_no_improve)
            if feasible
...
function replaceFromList(inst::Instance, this_sol::Sol, allparam::AllParameters, paramchosen, boats_to_be_visited, reconstruct_no_improve)
    @unpack N, P, Pi, visits, shipsIn, shipsOut, h, dist, delta, qli, T, Bp, maxT = inst
...
function localSearchRemovalReplace(inst::Instance, this_sol::Sol, allparam, paramfixed, paramchosen, reconstruct_no_improve)
    @unpack N, P, Pi, visits, shipsIn, shipsOut, h, dist, delta, qli, T, Bp, maxT = inst
    sol=deepcopy(this_sol)
    ...
    start = time_ns()
    sol, feasible = replaceFromList(inst, sol, allparam, paramchosen, boats_to_be_visited, reconstruct_no_improve)
    if feasible
...
>>>>>>> 14f2bf4d6d5e7d5d0910d196fae52a875eea4dce
        start_step = time_ns()
