...
function SelectNewVisitPerShip(inst::Instance, sol::Sol, paramchosen::ChosenParameters, allparam::AllParameters, n, c, p, printdetails) 
    ...
    ship_type = shipsIn[n].type
    tactic=paramchosen.TacticTypeShip[ship_type]
    if length(new_visits)>0
        if tactic=="cost"
            alpha_value= Alpha.CostTypeShip[ship_type][paramchosen.IndexTypeShip[ship_type]]
        ...
        if tactic=="dist"
            alpha_value= Alpha.DistanceTypeShip[ship_type][paramchosen.IndexTypeShip[ship_type]]
            list_available_shippos = Vector{NewVisit}()
            for visit in new_visits
                if visit.constrained==false
...
function ChooseParamExp(allparam::AllParameters, N, type1, type2)
    ...
    tactictypeship = ["" for i in 1:N]
    IndexTypeShip = [0 for i in 1:N]
    for n in 1:3
        if type1=="both"
            tactictypeship[n] = sample(["cost","dist"], Weights(Proba.ChooseTacticTypeShip[n]))
        end
        if type1=="cost"
            tactictypeship[n]="cost"
        end
        if type1=="dist"
            tactictypeship[n]="dist"
        end
        if tactictypeship[n]=="cost"
            IndexTypeShip[n] = sample(1:n_proba_typeship_cost+1, Weights(Proba.CostTypeShip[n]))
        else
            IndexTypeShip[n] = sample(1:n_proba_typeship_distance+1, Weights(Proba.DistanceTypeShip[n]))
        end
    end
    if type2=="both"
    ...
    return ChosenParameters(tactictypeship,IndexTypeShip,tacticallship,indexallship, indexconstrained)
end
...
function UpdateParametersExp(paramchosen::ChosenParameters, allparam::AllParameters, N, cost, new_cost)
    ...
    for n in 1:3
        if paramchosen.TacticTypeShip[n]=="cost"
            nbexp = allparam.Nbexp.ChooseTacticTypeShip[n][1]
            newparam.AverageCost.ChooseTacticTypeShip[n][1] = (allparam.AverageCost.ChooseTacticTypeShip[n][1]*nbexp+new_cost)/(nbexp+1)
            newparam.Nbexp.ChooseTacticTypeShip[n][1]=nbexp+1
            for i in 1:2
                newav = deepcopy(newparam.AverageCost.ChooseTacticTypeShip[n][i])
                newparam.Q.ChooseTacticTypeShip[n][i]=cost^2/newav^2
            end
            for i in 1:2
                newq=deepcopy(newparam.Q.ChooseTacticTypeShip[n][i])
                newparam.Proba.ChooseTacticTypeShip[n][i] = newq/sum(newparam.Q.ChooseTacticTypeShip[n])
            end
            index = paramchosen.IndexTypeShip[n]
            nbexp = allparam.Nbexp.CostTypeShip[n][index]
            newparam.AverageCost.CostTypeShip[n][index] = (allparam.AverageCost.CostTypeShip[n][index]*nbexp+new_cost)/(nbexp+1)
            newparam.Nbexp.CostTypeShip[n][index]=nbexp+1
            for i in 1:n_proba_typeship_cost+1
                newav = deepcopy(newparam.AverageCost.CostTypeShip[n][i])
                newparam.Q.CostTypeShip[n][i]=cost^2/newav
            end
            for i in 1:n_proba_typeship_cost+1
                newq=deepcopy(newparam.Q.CostTypeShip[n][i])
                newparam.Proba.CostTypeShip[n][i] = newq/sum(newparam.Q.CostTypeShip[n])
            end
        end
        if paramchosen.TacticTypeShip[n]=="dist"
            nbexp = newparam.Nbexp.ChooseTacticTypeShip[n][2]
            newparam.AverageCost.ChooseTacticTypeShip[n][2] = (allparam.AverageCost.ChooseTacticTypeShip[n][2]*nbexp+new_cost)/(nbexp+1)
            newparam.Nbexp.ChooseTacticTypeShip[n][2]=nbexp+1
            for i in 1:2
                newav = deepcopy(newparam.AverageCost.ChooseTacticTypeShip[n][i])
                newparam.Q.ChooseTacticTypeShip[n][i]=cost^2/newav^2
            end
            ...
            index = paramchosen.IndexTypeShip[n]
            nbexp = allparam.Nbexp.DistanceTypeShip[n][index]
            newparam.AverageCost.DistanceTypeShip[n][index] = (allparam.AverageCost.DistanceTypeShip[n][index]*nbexp+new_cost)/(nbexp+1)
            newparam.Nbexp.DistanceTypeShip[n][index]=nbexp+1
            for i in 1:n_proba_typeship_distance+1
                newparam.Q.DistanceTypeShip[n][i]=cost^2/newparam.AverageCost.DistanceTypeShip[n][i]^2
            end
            for i in 1:n_proba_typeship_distance+1
                newparam.Proba.DistanceTypeShip[n][i] = newparam.Q.DistanceTypeShip[n][i]/sum(newparam.Q.DistanceTypeShip[n])
            end
        end
    end
...
function UpdateParameters(paramchosen::ChosenParameters, allparam::AllParameters, N, cost, new_cost)
    ...
    for n in 1:N
        if paramchosen.TacticTypeShip[n]=="cost"
            ...
            for i in 1:2
                newav = deepcopy(newparam.AverageCost.ChooseTacticTypeShip[n][i])
                newparam.Q.ChooseTacticTypeShip[n][i]=cost/newav
            end
            for i in 1:2
                newq=deepcopy(newparam.Q.ChooseTacticTypeShip[n][i])
                newparam.Proba.ChooseTacticTypeShip[n][i] = newq/sum(newparam.Q.ChooseTacticTypeShip[n])
            ...
            for i in 1:n_proba_typeship_cost+1
                newav = deepcopy(newparam.AverageCost.CostTypeShip[n][i])
                newparam.Q.CostTypeShip[n][i]=cost/newav
            end
            for i in 1:n_proba_typeship_cost+1
                newq=deepcopy(newparam.Q.CostTypeShip[n][i])
                newparam.Proba.CostTypeShip[n][i] = newq/sum(newparam.Q.CostTypeShip[n])
        ...
        if paramchosen.TacticTypeShip[n]=="dist"
            ...
            newparam.Nbexp.DistanceTypeShip[n][index]=nbexp+1
            for i in 1:n_proba_typeship_distance+1
                newparam.Q.DistanceTypeShip[n][i]=cost/newparam.AverageCost.DistanceTypeShip[n][i]
            end
            for i in 1:n_proba_typeship_distance+1
                newparam.Proba.DistanceTypeShip[n][i] = newparam.Q.DistanceTypeShip[n][i]/sum(newparam.Q.DistanceTypeShip[n])
            end
...
function GRASP_reactive(inst::Instance, type1, type2, adjustproba, alpha_localsearch, time_local, max_time_heur, max_time)
...
sol, cost, allparam = GRASP_reactive(inst, "both","both", AdjustProba(2,3,2,2,2),  4, 2, 30,120)
print('\n')
print(cost)
print('\n')
print("Proba tactic type :")
print('\n')
print(allparam.Proba.ChooseTacticTypeShip)
print('\n')
