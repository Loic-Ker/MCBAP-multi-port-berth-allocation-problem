...
function localSearchRemovalReplace(inst::Instance, this_sol::Sol, allparam)
    @unpack N, P, Pi, visits, shipsIn, shipsOut, h, dist, delta, qli, T, Bp, maxT = inst
    sol=deepcopy(this_sol)
    nb_to_remove = ceil(Int, 0.2*N)
    ...
    random_removals = sample(random_removals, ceil(Int, 0.2*length(random_removals)); replace=false)
