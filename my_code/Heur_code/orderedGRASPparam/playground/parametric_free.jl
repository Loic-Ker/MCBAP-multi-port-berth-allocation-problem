...
function GRASP_reactive(seed,N,Nout,qli, type1, type2, type3, paramfixed, temperature, time_local, max_time_heur, max_time, expname, location)
    ...
    while elapsed<max_time
        ...
        if feasible && checkSolutionFeasability(inst, new_sol)
            ...
            if new_cost_heur<min_cost_heur+0.1*min_cost_heur && nb_iter_reconstruct>10
                ...
                new_sol, new_cost, delay_cost, waiting_cost, penalty_cost, handling_cost, fuel_cost = manualLocalSearch(inst, new_sol, new_cost_heur, delay_cost_heur, waiting_cost_heur, penalty_cost_heur, handling_cost_heur, fuel_cost_heur, allparam, time_local)
            ...
            if from_reconstruct==true
                if best_reconstruct_cost>new_cost
            ...
            from_reconstruct=false
            #if new_cost<min_cost_heur && nb_iter>10
            #    usedcplex=1
            #    print('\n')
    ...
    if feasible && checkSolutionFeasability(inst, sol)
        ...
        allparam = initializeParam()
        sol=initializeSol(inst, allparam)
        return best_sol, best_cost, allparam
    end
end
