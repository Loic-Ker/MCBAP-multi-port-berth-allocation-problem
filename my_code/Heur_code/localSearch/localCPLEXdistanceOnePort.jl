...
function local_search(inst::Instance, sol::Sol, alpha, max_time)
    ...
    for n in 1:N
        for c in 1:length(inst.Pi[n])
            append!(list_initialize,[[n,c]])
        end
    end
    start = time_ns()
    elapsed = round((time_ns()-start)/1e9,digits=3)
    while elapsed<max_time && length(list_initialize)>0.2*length(inst.Pi)
        new_visit=rand(list_initialize)
        other_visits=get_closest(inst,new_sol,new_visit,alpha)
        ...
        if checkSolutionFeasability(inst, new_solCPLEX)
            ...
            new_cost=checkSolutionCost(inst, new_sol)
            list_initialize=setdiff(list_initialize, [new_visit])
        end
        elapsed = round((time_ns()-start)/1e9,digits=3)
    end
...
function get_closest(inst::Instance, sol::Sol, new_visit, alpha)
    @unpack N, Ntot, P, Pi, visits, shipsIn, shipsOut, h, dist, delta, qli, T, Bp, maxT, Nl, gamma, Hc, Dc, Fc, Ic, Pc, beta, ports = inst
    all_visits_distance=[]
    for n in 1:N
        for c in 1:length(inst.Pi[n])
            if inst.Pi[n][c]==inst.Pi[new_visit[1]][new_visit[2]]
                distance_squares=abs(sol.visits[n][c].b-sol.visits[new_visit[1]][new_visit[2]].b)+abs(sol.visits[n][c].t-sol.visits[new_visit[1]][new_visit[2]].t)
