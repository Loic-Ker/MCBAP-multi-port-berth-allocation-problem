...
function local_search(inst::Instance, sol::Sol, cost::Int64, allparam::AllParameters, paramchosen::ChosenParameters, alphaboat, alpharandom, max_time)
    ...
    for n in 1:N
        for c in 1:length(inst.Pi[n])
            append!(list_initialize_all,[[n,c]])
        end
    end
    start = time_ns()
    ...
    while elapsed<max_time && length(list_initialize_boats)>0 && length(list_initialize_all)>0
        ...
        if random_float> allparam.Alpha.ChooseTacticLocalSearch[paramchosen.ChooseTacticLocalSearch]
            tactic="Boat"
        else
            tactic="Random"
        end
        ...
        if tactic=="Boat"
            new_boat=rand(list_initialize_boats)
            other_visits=get_closest(inst,new_sol,new_boat,alphaboat)
        else
            other_visits=sample(list_initialize_all, min(alpharandom, length(list_initialize_all)); replace=false)
        end
        ...
        if checkSolutionFeasability(inst, new_solCPLEX)
            ...
            if tactic=="Boat"
                ...
                list_initialize_all=setdiff(list_initialize_all, [rand(other_visits)])
            end
        end
        elapsed = round((time_ns()-start)/1e9,digits=3)
    end
