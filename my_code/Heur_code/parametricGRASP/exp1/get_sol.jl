...
function makeExpText(type1, type2, type3, paramfixed, time_local, max_time_heur, max_time, expname, location)
    filename = location*"MCBAP-multi-port-berth-allocation-problem/results_jobs/benchmarks_HEUR/parametricGRASP/$expname"*"/explanations.txt"
    txttype1="The tactic for each ship : "*type1
    txttype2="The tactic between the ships : "*type2
    txttype2="The tactic for the local search : "*type3

    oneboatcostalpha = paramfixed.OneBoatCost
    oneboatdistalpha = paramfixed.OneBoatDist
    oneboattimealpha = paramfixed.OneBoatTime
    allboatcostalpha = paramfixed.AllBoatCost
    allboatcountalpha = paramfixed.AllBoatCount
    allboattimealpha = paramfixed.AllBoatTime
