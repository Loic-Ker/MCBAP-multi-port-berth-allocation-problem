...
def getgraphs_port(algo_folder, exp, seed,Nin,Nout,qli,type,time_limit):
    results = getsolfromfileHEUR(algo_folder=algo_folder, exp=exp, seed=seed,Nin=Nin,Nout=Nout,qli=qli)
    resultsCPLEX = getsolfromfileCPLEX(seed=seed,Nin=Nin,Nout=Nout,qli=qli,type=type,time_limit=time_limit)
    color =get_cmap(Nin)
...
def getgraphs_port_withiter(algo_folder, exp, seed,Nin,Nout,qli,nbiter, type, time_limit, withcplex=True):
    results = getspecificiterfromfileHEUR(algo_folder=algo_folder, exp=exp, seed=seed,Nin=Nin,Nout=Nout,qli=qli,iternb=nbiter)
    resultsCPLEX = getsolfromfileCPLEX(seed=seed,Nin=Nin,Nout=Nout,qli=qli, type=type, time_limit=time_limit)
    color =get_cmap(Nin)
    if withcplex:
        fig, axs = plt.subplots(3,2,figsize=(15, 15),sharex=True, sharey=True)
        ...
        for port in [1,2,3]:
            ...
            axs[0].set_title("Heuristics, cost : {} $".format(int(results["objectif"])))  
            for port in [1,2,3]:
                axs[port-1].set_ylabel("Port {}".format(port))
